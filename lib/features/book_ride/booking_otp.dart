...
class _BookingOTPState extends State<BookingOTP> {
  ...
  @override
  void initState() {
    print("Booking id is ${widget.bookingId}");
    super.initState();
    initMarkers();
                    ...
                    controller.animateCamera(
                      CameraUpdate.newLatLngZoom(
                          ...
                          getZoomLevel(
                            ...
                            LatLng(widget.droplatlong.latitude,
  ...
  Widget _renderAssignedDriver(Bookings model) {
    print("Booking id is ${model.bookingId}");
    return ListView(
      shrinkWrap: true,
      children: [
        Row(
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            RatingBar.builder(
              initialRating: double.parse(model.driverRating ?? "0"),
              minRating: 0,
    ...
    if (mounted) {
      _timer = Timer.periodic(
        ...
        (timer) async {
          ...
          if (context.read<AfterBookingController>().booking != null) {
            if (context.read<AfterBookingController>().booking!.status ==
                "payment done") {
              _timer!.cancel();
            } else if (context.read<AfterBookingController>().booking!.status ==
                "cancel") {
              _timer!.cancel();
            }
          }
  ...
  void _handleRideStatus(Timer timer) {
    ...
    if (context.read<AfterBookingController>().booking?.status == 'arrived') {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        ...
        content: const Text(
          "driver is arrived at your location\n\n",
        ),
...
double getZoomLevel(LatLng start, LatLng from, double screenWidth) {
  double zoomLevel = 0;
  var distance = mt.SphericalUtil.computeDistanceBetween(
    mt.LatLng(start.latitude, start.longitude),
    mt.LatLng(from.latitude, from.longitude),
  );

  if (screenWidth > 0) {
    double mapScale = distance / screenWidth;
    zoomLevel = (16 - log(mapScale) / log(2));
  }
